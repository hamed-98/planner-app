      ...
      <div className="bg-gradient-to-r from-slate-900 to-teal-950 p-6 rounded-3xl text-white shadow-xl relative overflow-hidden">
        ...
        <div className="relative">
          ...
          <p className="text-xs text-slate-350">
            {dateText} | شما در این روز {completedTodayCount} کار را تکمیل
            کردید.
          </p>
        </div>
      </div>
              ...
              <button
                title={
                  selectedDateISO > todayISO
                    ? "امکان تحلیل برای آینده وجود ندارد"
                    : "به‌روزرسانی تحلیل"
                }
                disabled={
                  selectedDateISO > todayISO ||
          ...
          {userWeight > 0 && userHeight > 0 ? (
            ...
            <div className="text-center py-5">
              ...
              <p className="text-[10px] text-teal-600 font-medium mt-1">
                قد و وزن را در تب سلامت ثبت کنید
              </p>
            </div>
          )}
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-white dark:bg-slate-900 p-6 rounded-3xl border border-slate-100 dark:border-slate-800 shadow-sm">
          ...
          <div className="space-y-3">
            {tasks
              .filter(
                (t) => t.status !== "done" && t.dueDate === selectedDateISO,
              )
              .slice(0, 3)
              .map((task) => (
                <div
